...
def plot_feature_missingness(data):
    ...
    # Histogram
    ax = axes[0]
    ax.hist(missing_features_nonzero.values, bins=30, color="mediumvioletred", edgecolor="white", linewidth=0.5)
    ax.set_xlabel("Missing (%)")
    ax.set_ylabel("Number of features")
    ax.set_title(f"Distribution of missingness\n({len(missing_features_nonzero)} features with any missing)")
    ax.axvline(x=20, color="black", linestyle="--", linewidth=0.8, label="20% threshold")
    ax.legend()
    ...
    ax.axhline(y=20, color="black", linestyle="--", linewidth=0.8, label="20% threshold")
    ax.legend()
...
def missingness_summary(df_before, df_after):
    ...
    print(f"Features affected : {(df_before.isnull().any()).sum()} / {df_before.shape[1]}")
    print(f"Samples affected  : {(df_before.isnull().any(axis=1)).sum()} / {df_before.shape[0]}")
...
def plot_intensity_distribution(data):
    ...
    fig, (ax_nan, ax_hist) = plt.subplots(1, 2, figsize=(11, 5),
    ...
    ax_hist.hist(log_values, bins=100, color="cornflowerblue", edgecolor="none", alpha=0.8)
...
data_path = (
    "../../example_data/DidacMauricio_hilic/DM_FIS2018_Hilic_pos_results2023_filtered.csv"
)
...
print(f"Total count of missing cells: {data.isnull().sum().sum()}")  # total missing cells
print(f"Overall percentage of missingness: {data.isnull().mean().mean() * 100}\n")  # overall % missing
...
# %%
data_imputed_zeros = imputation_zeros(
    data=data
)

# %%
# Total missing count and percentage
print(f"Total count of missing cells: {data_imputed_zeros.isnull().sum().sum()}")  # total missing cells
print(f"Overall percentage of missingness: {data_imputed_zeros.isnull().mean().mean() * 100}\n")  # overall % missing
...
# %%
# Total missing count and percentage
print(f"Total count of missing cells: {data_imputed_hm.isnull().sum().sum()}")  # total missing cells
print(f"Overall percentage of missingness: {data_imputed_hm.isnull().mean().mean() * 100}\n")  # overall % missing
