    ...
    if len(group) > 0:
        for g in group:
            if not isinstance(g, Iterable):
                raise TypeError("Each element in the group must be an iterable (set, list, or tuple).")
    elif len(group) == 0:
...
def btwn_jaccard(group1: Iterable, group2: Iterable) -> tuple:
    ...
    for g1, g2 in itertools.product(group1, group2):
        ...
        if not isinstance(g1, Iterable) or not isinstance(g2, Iterable):
            raise TypeError("Each element in the groups must be an iterable (set, list, or tuple).")
